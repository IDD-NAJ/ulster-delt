...
export default function UserTableClient({ users: initialUsers }: UserTableClientProps) {
  ...
  const [txForm, setTxForm] = useState({ id: '', type: '', amount: '', description: '' });
  ...
  const openTransactionDialog = async (user: User, account: Account) => {
    ...
    setTxForm({ id: '', type: '', amount: '', description: '' });
    setEditTxId(null);
  };
  ...
  const handleEditTxClick = (tx: any) => {
    setTxForm({ id: tx.id, type: tx.type, amount: tx.amount.toString(), description: tx.description || '' });
    setEditTxId(tx.id);
  };
      ...
      <Dialog open={deleteUserDialogOpen} onOpenChange={setDeleteUserDialogOpen}>
        <DialogContent>
          ...
          <div className="flex justify-end gap-2 mt-4">
            ...
            <Button type="button" variant="destructive" onClick={() => userToDelete && handleDeleteUser(userToDelete.id)} disabled={loading}>
              Delete
            </Button>
          </div>
        </DialogContent>
